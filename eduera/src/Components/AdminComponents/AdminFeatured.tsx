...
// export const AdminFeatured = ({userData} : any) => {


//     const studentTotal = userData.reduce((total : any, el : any) => {
//       const Students = el.students;
//       const totalStudents = parseInt(Students);
//       return total + totalStudents;
//     }, 0);
    
//     const facultyTotal = userData.reduce((total : any, el : any) => {
...
export const AdminFeatured: React.FC<AdminFeaturedProps> = ({ userData }) => {
  const studentTotal = userData.reduce((total, el) => {
    const Students = el.students;
    const totalStudents = Number(Students);
    return total + totalStudents;
  }, 0);

  const facultyTotal = userData.reduce((total, el) => {
    const Faculty = el.faculty;
    const totalFaculty = Number(Faculty);
    return total + totalFaculty;
  }, 0);
...
const FeaturedIcon = styled(ArrowUpward)<{ up: boolean }>`
  font-size: 14px;
  color: ${({ up }) => (up ? "green" : "red")};
`;
